...
export default function ProductCard({
  ...
  const [productCategory, setProductCategory] = useState(String(categoryId ?? ""));
  const [categories, setCategories] = useState([]);
  const [imageFile, setImageFile] = useState(null);
  const [imagePreview, setImagePreview] = useState(buildProductImageUrl(imagePath));
  ...
  }, [sellprice, name, description, categoryId, imagePath]);

  const handleEdit = async () => {
    const numericPrice = Number(newSellPrice);

    if (!id || !productName.trim() || !productCategory || !Number.isFinite(numericPrice) || numericPrice < 0) {
  ...
  return (
    <article className="flex h-full w-full min-h-[360px] flex-col overflow-hidden rounded-2xl bg-white p-3.5 shadow-md relative">
      <div className="flex h-48 w-full shrink-0 items-center justify-center overflow-hidden rounded-xl bg-gray-50 p-2">
        <img
          ...
          onError={(event) => {
            event.currentTarget.src = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTULlOeY6XTrnI_PT7ypqVrR-dHQghz7qnQxEV5IwZzrw&s";
          }}
        />
      </div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        ...
        {/* Edit Right-side Slide-over Drawer */}
        <div
          className={`fixed inset-y-0 right-0 z-[110] w-full max-w-[480px] bg-white shadow-2xl transform transition-transform duration-300 ease-in-out flex flex-col text-left ${
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
            ...
            <TextField className="w-full" name="selling-price">
              <Label className="text-gray-700 font-medium mb-1.5 text-sm">Selling Price</Label>
              <InputGroup>
